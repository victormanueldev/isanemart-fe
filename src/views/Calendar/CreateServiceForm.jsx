...
import { handleToggleModal, startCreatingService } from '../../state';
...
export const CreateServiceForm = () => {
  ...
  const { headquarters } = useSelector((state) => state.headquarters);
  const [headquartersToShow, setHeadquartersToShow] = useState(headquarters);

  const onCloseModal = () => {
    dispatch(handleToggleModal());
  };

  const onCustomerChange = ({ target }) => {
    const customerId = target.value;
    setHeadquartersToShow(
      headquarters.filter((headquarter) => headquarter.customer_id === parseInt(customerId))
    );
  };
  ...
  const onFormSubmit = (data) => {
    setStatus(!status);
    const serviceData = {
      customer_id: parseInt(data.customer),
      headquarter_id: parseInt(data.headquarter),
    ...
    dispatch(startCreatingService(serviceData));
  };
            ...
            <div className="col-span-1">
              ...
              <div className="relative">
                <select
                  ...
                  className="py-3 px-4 block w-full border-gray-200 rounded-md text-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400"
                  {...register('headquarter', { required: false })}
                  placeholder="Selecciona un cliente"
                >
                  {headquartersToShow?.map((headquarter, index) => (
                    <option key={index} value={headquarter.id}>
                      {headquarter.name}
                    </option>
                  ))}
                </select>
